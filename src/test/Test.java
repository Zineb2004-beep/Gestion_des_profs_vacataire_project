...
public class Test {
    public static void main(String[] args) {
        ...
        afs.affecterCours(19, 20);
        afs.affecterCours(20, 19);
        
        
         
        Professeur pu = pfs.findById(20); 
        if (pu != null) {
            pu.setSpecialite("Economie"); 
            pfs.update(pu);
        }

        Cours cu = cs.findById(19); 
        if (cu != null) {
            cu.setSalle("Salle I3");
            cs.update(cu);
        }

        AffectationCours au = afs.findById(19, 20); 
        if (au != null) {
            au.setProf(pfs.findById(19)); 
            afs.update(au);
        }
        ...
        AffectationCours af = afs.findById(20);
